...
class UtilCKEeditor extends Util 
{
    ...
    */
     ...
     * @param string $textarea_id 在线编辑器所在的内容编辑区域TextArea的ID                      
     */
    public static function loadReplace($textarea_id)
    {                                                                                    
        $result=self::load().UtilAjax::loadJsContentSentence(self::replace($textarea_id));    
        $result.=self::ckFinder();
        return $result;  
    }                   
     ...
     * @param string $textarea_id 在线编辑器所在的内容编辑区域TextArea的ID                      
     */
    public static function replace($textarea_id)
    {
        if (self::$CKEditor==null){
            $CKEditor = new CKEditor();  
        }
        ...
        $config['startupOutlineBlocks'] = true;    
        $config['removeDialogTabs'] = 'image:Link;image:advanced';    
        $jsContent= "var editor = CKEDITOR.replace('".$textarea_id."', ".$CKEditor->jsEncode($config).");";   
        $jsContent="\r\n". 
