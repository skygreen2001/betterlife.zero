...
{block name=body}
 <div class="block">
	...
	<div align="center"><my:a href='{$url_base}index.php?go=model.department.lists&pageNo={$smarty.get.pageNo|default:"1"}'>返回列表</my:a>{if $department}|<my:a href='{$url_base}index.php?go=model.department.view&id={$department.id}&pageNo={$smarty.get.pageNo|default:"1"}'>查看用户所属部门</my:a>{/if}</div>
</div>
{/block}