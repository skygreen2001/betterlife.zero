		...
		*/
		public static function get($filter=null)
		{
				...
				for ($i=0;$i<count($result);$i++) {
						if (!array_key_exists(Module_Loader::SPEC_REQUIRED,$result[$i]))
						{
								$result[$i][Module_Loader::SPEC_REQUIRED]=false;
						}else{
								if ($result[$i][Module_Loader::SPEC_REQUIRED]=='true'){
										$result[$i][Module_Loader::SPEC_OPEN]=Module_Loader::OPEN_YES;
										$result[$i][Module_Loader::SPEC_REQUIRED]=true;
								}else{
										$result[$i][Module_Loader::SPEC_REQUIRED]=false;
								}
						}      
						...
						foreach ($spec_library as $key=>$dataobjets){ 
								...
								foreach ($dataobjets as $key=>$block){  
										...
										if (isset($condition)&&(!self::isValidData($blockAttr,$condition))){    
												if (isset($blockAttr[Module_Loader::SPEC_REQUIRED])&&($blockAttr[Module_Loader::SPEC_REQUIRED]=='true')){
														if ($condition[Module_Loader::SPEC_OPEN]=="false"){
...
//                   else $filter_open=false; 
				...
				foreach ($resourceLibs as $key=>$value) {
						...
						if (!array_key_exists(Module_Loader::SPEC_REQUIRED,$resourceLibs[$key]))
						{
								$resourceLibs[$key][Module_Loader::SPEC_REQUIRED]=false;
						}else{
								if ($resourceLibs[$key][Module_Loader::SPEC_REQUIRED]=='true'){  
										$resourceLibs[$key][Module_Loader::SPEC_OPEN]=true; 
										$resourceLibs[$key][Module_Loader::SPEC_REQUIRED]=true;                                               
								}else{
										$resourceLibs[$key][Module_Loader::SPEC_REQUIRED]=false;                     
								}                  
						}  
