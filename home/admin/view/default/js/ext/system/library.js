	...
	libraryStore : new Ext.data.Store({
		...
		listeners : {
			'beforeload' : function(store, options) {
				if (Ext.isReady) {
					Bb.Library.Config.params = Bb.Library.Form.getForm()
							.getValues();
					Ext.apply(options.params, Bb.Library.Config.params);
				}
			}
		}
	})
...
Bb.Library.Grid = Ext.extend(Ext.grid.GridPanel, {
	constructor : function(config) {
		...
		config = Ext.apply({
					...
					tbar : [{
								...
								handler : function() {
									var gridCmp = Ext
											.getCmp(Bb.Library.Config.GridName);
									gridCmp.store.save();
								}
							}, {
								xtype : 'tbseparator'
	...
	onReverseSelect : function() {
		var gridCmp = Ext.getCmp(Bb.Library.Config.GridName);
		for (var i = gridCmp.getView().getRows().length - 1; i >= 0; i--) {
			if (gridCmp.sm.isSelected(i)) {
				gridCmp.sm.deselectRow(i);
			} else {
				gridCmp.sm.selectRow(i, true);
			}
		}
	},
	/**
	...
	onAdd : function(btn, ev) {
		var gridCmp = Ext.getCmp(Bb.Library.Config.GridName);
		var u = new gridCmp.store.recordType({
					name : '',
					init : '',
					open : false,
					required : false
				});
		gridCmp.editor.stopEditing();
		gridCmp.store.insert(0, u);
		gridCmp.getView().refresh();
		gridCmp.editor.startEditing(0, 0);
		gridCmp.getBottomToolbar().updateInfo();
	},
	/**
	 * 删除
	 */
	onDelete : function() {
		var gridCmp = Ext.getCmp(Bb.Library.Config.GridName);
		Ext.MessageBox.confirm('提示', '确实要删除所选的记录吗?', gridCmp.showResult);
	},
	/**
	 * 批量删除
	 */
	showResult : function showResult(btn) {
		...
		if (btn == 'yes') {
			...
			for (var i = 0, r; r = selectedRows[i]; i++) {
				...
				gridCmp.store.remove(r);
			}
		}
		gridCmp.store.save();
		gridCmp.getBottomToolbar().updateInfo();
		// gridCmp.getView().refresh();//刷新整个grid视图,重新排序.
	},
...
Ext.onReady(function() {
			 ...
			 * 保证分页也将查询条件带上
			 */
			// Bb.Library.Store.libraryStore.on('beforeload',function(store,options)
			// {
