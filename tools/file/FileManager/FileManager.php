<?php
//a:9:{s:4:"lang";s:2:"zh";s:9:"auth_pass";s:32:"d41d8cd98f00b204e9800998ecf8427e";s:8:"quota_mb";i:0;s:17:"upload_ext_filter";a:0:{}s:19:"download_ext_filter";a:0:{}s:15:"error_reporting";s:0:"";s:7:"fm_root";s:0:"";s:17:"cookie_cache_time";i:1096733048;s:7:"version";s:5:"0.9.3";}
header("Content-Type:text/html; charset=UTF-8");  
require_once("../../../init.php");
//todo:中文文件夹无法打开|Breadcrumb左侧的两个小箭头第一次无法看见
/*--------------------------------------------------
 | PHP FILE MANAGER
 +--------------------------------------------------
...
// +--------------------------------------------------
	...
	$script_filename = substr($doc_root,0,strlen($doc_root)-1).$_SERVER["PHP_SELF"];       
...
// +--------------------------------------------------
	...
	if (!isset($expanded_dir_list)){
		$expanded_dir_list = "";
		$nav_root_path=str_replace("\\","/",Gc::$nav_root_path);//$path_info["dirname"]
		$mat = explode("/",$nav_root_path);
	...
	if (!isset($fm_root_atual)){
		if (strlen($fm_root)) $fm_root_atual = $fm_root;
			...
			$nav_root_path=str_replace("\\","/",Gc::$nav_root_path);//$path_info["dirname"]."/"
			if (!$islinux) $fm_root_atual = ucfirst($nav_root_path);
...
function et($tag){
	...
	$zh['NoSel'] = '请先选择至少一个文件或者文件夹';
	$zh['SelDir'] = '在左侧目录树里选择需查看的文件夹';
	$zh['TypeDir'] = '输入文件夹名称';
	$zh['TypeArq'] = '输入文件名称';
	$zh['TypeCmd'] = '请输入shell命令';
	$zh['TypeArqComp'] = '输入文件名.\\n文件扩展名会定义压缩的类型.\\nEx:\\nnome.zip\\nnome.tar\\nnome.bzip\\nnome.gzip';
	$zh['RemSel'] = '删除已选择的文件和文件夹';
	$zh['NoDestDir'] = '没有选择需查看的文件夹';
	$zh['DestEqOrig'] = '源文件夹和目标文件夹一样';
	$zh['InvalidDest'] = '目标文件夹不合法';
	$zh['NoNewPerm'] = '新权限没有设置成功';
	$zh['CopyTo'] = '复制到';
	$zh['MoveTo'] = '移动到';
	$zh['AlterPermTo'] = '修改权限到';
	$zh['ConfExec'] = '确认 执行';
	$zh['ConfRem'] = '确认 删除';
	$zh['EmptyDir'] = '空文件夹';
	$zh['IOError'] = 'I/O 错误';
	...
	$zh['TypePass'] = '请输入密码';
	$zh['InvPass'] = '非法的密码';
	$zh['ReadDenied'] = '无权限读文件';
	$zh['FileNotFound'] = '无法找到文件';
	$zh['AutoClose'] = '完成自动关闭';
	$zh['OutDocRoot'] = '文件超出了 DOCUMENT_ROOT';
	$zh['NoCmd'] = '错误: 无法传送命令到服务器';
	$zh['ConfTrySave'] = '文件没有写权限.\\n重试保存';
	$zh['ConfSaved'] = '配置已保存';
	$zh['PassSaved'] = '密码已重置';
	$zh['FileDirExists'] = '文件或文件夹已存在';
	$zh['NoPhpinfo'] = '无法查看 phpinfo 函数显示信息';
	$zh['NoReturn'] = '没有返回';
	$zh['FileSent'] = '发送文件';
	$zh['SpaceLimReached'] = '无空闲的空间';
	$zh['InvExt'] = '非法的文件扩展名';
	$zh['FileNoOverw'] = '文件不能被重写';
	$zh['FileOverw'] = '文件重写';
	$zh['FileIgnored'] = '文件被忽略';
	$zh['ChkVer'] = '在 sf.net 网站上检查是否有新版本';
	$zh['ChkVerAvailable'] = '新版本, 点选这里开始下载!!';
	$zh['ChkVerNotAvailable'] = '没有新版本. :(';
	$zh['ChkVerError'] = '连接错误.';
	$zh['Website'] = '网站';
...
function download(){
	...
	$file_show=$dir_atual.$filename;
	$file = UtilString::utf82gbk($file_show);
	if(file_exists($file)){
		$is_proibido = false;
		...
		if (!$is_proibido){
			...
			if ($fh = fopen("$file", "rb")){
				fpassthru($fh);
				fclose($fh);
			} else alert(et('ReadDenied').": ".$file_show);
		} else alert(et('ReadDenied').": ".$file_show);
	} else alert(et('FileNotFound').": ".$file_show);
}
function execute(){
	global $cmd;
...
function save_upload($temp_file,$filename,$dir_dest) {
	...
	$file = UtilString::utf82gbk($dir_dest.$filename);
			...
			while ($file = readdir($handle)){
				if ($file != "." && $file != ".." && is_dir("$dir_corrente/$file")){
					...
					if (!UtilString::is_utf8($file)){
						$filenametmp = UtilString::gbk2utf8($file);
					}
					$mat_dir[] =$filenametmp;        
				}                        
			}
...
function show_tree(){
	...
	<!--
		...
		if (is.ie) document.onselectstart=new Function('return false')
			...
			document.onmousedown=disableTextSelection(null);
	...
	echo "<tr valign=\"top\" height=\"10\"><td><form>";
	if (!$islinux){
		...
		for($x=0;$x<strlen($aux);$x++){
			...
			echo "<option $is_sel value=\"".$aux[$x].":/\">".$aux[$x].":/</option>";
		}
		echo "</select> ";
	}
	echo "<input type=button value=".et('Refresh')." onclick=\"atualizar()\" /></form></td></tr>";
	...
	echo "<form name=\"login_form\" action=\"".$path_info["basename"]."\" method=\"post\" target=\"_parent\">
		  <tr><td height=\"10\" colspan=\"2\"><input type=\"hidden\" name=\"action\" value=\"1\" /><input type=\"submit\" value=\"".et('Leave')."\" /></tr></form>";
	echo "</table>\n"; 
	echo "</body>\n</html>";
}
...
function dir_list_form() {
	...
	$dir_atual = UtilString::utf82gbk($dir_atual);
	if ($opdir = @opendir($dir_atual)) {
		...
		while ($file = readdir($opdir)) {
		  if (($file != ".")&&($file != "..")){
			...
			$file_show=$file;
			if (!UtilString::is_utf8($file))$file_show=UtilString::gbk2utf8($file_show);
			$entry_list[$entry_count]["name"] = $file_show;
			...
			$entry_list[$entry_count]["datet"] = date("Y年m月d日 H:i:s", filemtime($dir_atual.$file));
		...
		$dir_atual = UtilString::gbk2utf8($dir_atual);
		$out .= "
		<script language=\"Javascript\" type=\"text/javascript\">
		<!--
		...
		function download(arg){
				var w = 10;
				var h = 10;
				window.open('".$path_info["basename"]."?action=3&filename='+escape(arg)+'&dir_atual=$dir_atual', '', 'width='+w+',height='+h+',fullscreen=no,scrollbars=no,resizable=yes,status=no,toolbar=no,menubar=no,location=no');
		}
		function upload(){
				var w = 400;
				var h = 400;
		...
		function rename(arg){
				var nome = '';
				if (nome = prompt('".strtoupper(et('Ren'))." \\' '+arg+' \\' ".et('To')." ...',arg)) document.location.href='".$path_info["basename"]."?frame=3&action=3&dir_atual=$dir_atual&old_name='+escape(arg)+'&new_name='+escape(nome);
		}
		...
		//if ($dir_atual != $fm_root_atual){
            ...
            if (!empty($dir_antes))$uplink = "<a href=\"".$path_info["basename"]."?frame=3&dir_atual=$dir_antes\"><<</a> ";
		//}
		if($entry_count){
			$out .= "
				<tr><td style=\"white-space: nowrap;\" bgcolor=\"#DDDDDD\" colspan=20><nobr>$uplink <a href=\"".$path_info["basename"]."?frame=3&dir_atual=$dir_atual\">$dir_atual</a></nobr></td></tr>
				<tr>
				<td bgcolor=\"#DDDDDD\" colspan=20><nobr>
			...
			foreach ($entry_list as $ind=>$dir_entry) {
				 ...
				 if ($dir_entry["type"]=="dir"){
					 ...
					 // Opções de diretório
					 $dir_actual_file = UtilString::utf82gbk($dir_atual.$file);   			
					 if ( is_writable($dir_actual_file) ) $dir_out .= "
					 ...
					 // Opções de arquivo
					 $dir_actual_file = UtilString::utf82gbk($dir_atual.$file);
					 if ( is_writable($dir_actual_file) ) $file_out .= "
								 ...
								 <td bgcolor=\"#FFFFFF\" align=center><a href=\"javascript:rename('$file')\">".et('Ren')."</a>";
					 if ( is_readable($dir_actual_file) && (strpos(".sys#.wav#.mp3#.mid#.avi#.mov#.mpeg#.mpg#.rm#.iso#.bin#.img#.dll#.psd#.fla#.swf#.class#.ppt#.jpg#.gif#.png#.wmf#.eps#.bmp#.msi#.exe#.com#.rar#.tar#.zip#.bz2#.tbz2#.bz#.tbz#.bzip#.gzip#.gz#.tgz#", $dir_entry["ext"]."#" ) === false)) $file_out .= "
								 <td bgcolor=\"#FFFFFF\" align=center><a href=\"javascript:edit_file('$file')\">".et('Edit')."</a>";
					 if ( is_readable($dir_actual_file) && strlen($dir_entry["ext"]) && (strpos(".tar#.zip#.bz2#.tbz2#.bz#.tbz#.bzip#.gzip#.gz#.tgz#", $dir_entry["ext"]."#" ) !== false)) $file_out .= "
								 <td bgcolor=\"#FFFFFF\" align=center><a href=\"javascript:decompress('$file')\">".et('Decompress')."</a>";
					 if( $is_reachable && is_readable($dir_actual_file) && (strpos(".txt#.ini#.conf#.swf#.html#.htm#.jpg#.gif#.png#.bmp#", $dir_entry["ext"]."#" ) !== false)) $file_out .= "
...
function upload_form(){
	...
	if (count($_FILES)==0){
		...
		<tr><td align='right' width='90px'><b>".et('Destination').":<td><b><nobr>$dir_atual</nobr>";
...
function view(){
	...
	if ($is_reachable){
		...
		if ($_SERVER["HTTP_REFERER"]){
		  ...
		  $url_infom = parse_url(Gc::$url_base);                 
		  $domainurl="http://".$url_infom["host"]."/";
		  $url=$domainurl.substr($url,3);
		}        
		echo "                        
...
function edit_file_form(){
	...
	$file_read = UtilString::utf82gbk($file);
	if ($save_file){
		$fh=fopen($file_read,"w");
		fputs($fh,$file_data,strlen($file_data));
		fclose($fh);
	}
	$file_data=file_get_contents($file_read);
	html_header();
	$encode_arr = array('UTF-8','ASCII','GBK','GB2312','BIG5','JIS','eucjp-win','sjis-win','EUC-JP');
	$encoded = mb_detect_encoding($file_data, $encode_arr);
	$file_data = mb_convert_encoding($file_data,'UTF-8',$encoded);	
	echo "<body marginwidth=\"0\" marginheight=\"0\">
	...
	<tr><td colspan=2><textarea name='file_data' rows='33' cols='105'>$file_data</textarea></td></tr>
...
function shell_form(){
	...
	switch ($shell_form){
		...
		case 2:
			...
			$echo_result=str_replace("<","[",str_replace(">","]",str_replace("\n","\\n",str_replace("\\","\\\\",$data_out))));////htmlencode()
			if (!UtilString::is_utf8($echo_result)){$echo_result=UtilString::gbk2utf8($echo_result);}
			echo "
			<script language=\"Javascript\" type=\"text/javascript\">
			<!--";
...
function frame3(){
	...
	if ($action){
		switch ($action){
			case 1: // create dir
			if (strlen($cmd_arg)){
				$new_filedir=UtilString::utf82gbk($dir_atual.$cmd_arg);
				$cmd_arg = formatpath($new_filedir);
			...
			case 2: // create arq
			if (strlen($cmd_arg)){
				$cmd_arg = UtilString::utf82gbk($dir_atual.$cmd_arg);
				if (!file_exists($cmd_arg)){
			...
			if ((strlen($old_name))&&(strlen($new_name))){
				$old_name=unescape($old_name);
				$new_name=unescape($new_name);				
				$dir_atual_old_name=UtilString::utf82gbk($dir_atual.$old_name);
				$dir_atual_new_name=UtilString::utf82gbk($dir_atual.$new_name);
				rename($dir_atual_old_name,$dir_atual_new_name);
				if (is_dir($dir_atual_new_name)) reloadframe("parent",2);
			}
			break;
			case 4: // delete sel
				...
				if (strlen($selected_file_list)){
					...
					if (count($selected_file_list)) {
						for($x=0;$x<count($selected_file_list);$x++) {
							...
							$dir_atual=UtilString::utf82gbk($dir_atual);
							$selected_file=UtilString::utf82gbk($selected_file_list[$x]);
							if (strlen($selected_file_list[$x])) total_delete($dir_atual.$selected_file,$dir_dest.$selected_file);
						}
					}
				}
				if (strlen($selected_dir_list)){
					...
					if (count($selected_dir_list)) {
						for($x=0;$x<count($selected_dir_list);$x++) {
							...
							$dir_atual=UtilString::utf82gbk($dir_atual);
							$selected_dir=UtilString::utf82gbk($selected_dir_list[$x]);
							if (strlen($selected_dir_list[$x])) total_delete($dir_atual.$selected_dir,$dir_dest.$selected_dir);
						}
					}
				}
				$dir_atual=UtilString::gbk2utf8($dir_atual);
			...
			if (strlen($cmd_arg)){
				$delete_dir_file=UtilString::utf82gbk($dir_atual.$cmd_arg);
				if (file_exists($delete_dir_file))total_delete($delete_dir_file);
				if (is_dir($dir_atual)) reloadframe("parent",2);
			}
			break;
			case 9: // CHMOD
...
function frameset(){
	global $path_info;
	html_header();  
	$nav_root_path=str_replace("\\","/",Gc::$nav_root_path);                                       
	echo "
	<frameset cols=\"2,*\" framespacing=\"0\">
		...
		<frame src=\"".$path_info["basename"]."?frame=3&dir_atual=$nav_root_path\" name=frame3 border=\"0\" marginwidth=\"0\" marginheight=\"0\">
	</frameset>
	";
	echo "</html>";
}
